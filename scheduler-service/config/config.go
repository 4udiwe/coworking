...
type (
	...
	Kafka struct {
		...
		Topics  struct {
			SchedulerEvents string `env-required:"true" yaml:"booking_events" env:"KAFKA_BOOKING_EVENTS"`
	...
	Worker struct {
		WorkerBatchLimit int           `env-required:"true" yaml:"worker_batch_limit" env:"WORKER_BATCH_LIMIT"`
		WorkerInterval   time.Duration `env-required:"true" yaml:"interval" env:"WORKER_INTERVAL"`
	}
	Scheduler struct {

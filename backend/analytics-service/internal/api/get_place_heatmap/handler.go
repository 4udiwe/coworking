...
func (h *handler) Handle(ctx echo.Context, in Request) error {
	...
	cells, err := h.s.GetPlaceHeatmap(ctx.Request().Context(), in.PlaceID)

	if err != nil {
		return echo.NewHTTPError(http.StatusInternalServerError, err.Error())
	}
	return ctx.JSON(http.StatusOK, Response{HeatMap: lo.Map(cells, func(e entity.HeatmapCell, _ int) Cell { return Cell{Weekday: e.Weekday, Hour: e.Hour, Count: e.Count} })})
}

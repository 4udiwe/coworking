...
class BookingBloc extends Bloc<BookingEvent, BookingState> {
  ...
  BookingBloc({required this.repository}) : super(const BookingState()) {
    ...
    on<HighlightBooking>((event, emit) {
      emit(state.copyWith(highlightedBookingId: () => event.bookingId));
    });
    on<DisableBookingHighlight>((event, emit) {
      emit(state.copyWith(highlightedBookingId: () => null));
    });
  }

  Future<void> _fetchActive(
    FetchActiveBookings e,
    Emitter<BookingState> emit,
  ) async {
    emit(state.copyWith(activeLoading: true, activePage: 1));

    try {
      final res = await repository.getActiveBookings(
        page: 1,
        pageSize: 10,
      );
  ...
  Future<void> _fetchHistory(
    FetchHistoryBookings e,
    Emitter<BookingState> emit,
  ) async {
    emit(state.copyWith(historyLoading: true, historyPage: 1));

    try {
      final res = await repository.getHistoryBookings(page: 1, pageSize: 10);
  ...
  Future<void> _loadMoreHistory(
    ...
    final res = await repository.getHistoryBookings(page: next, pageSize: 10);

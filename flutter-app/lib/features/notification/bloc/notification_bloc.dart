...
class NotificationBloc
  ...
  Future<void> _onFetchNotifications(FetchNotifications event, Emitter<NotificationState> emit) async {
    ...
    if (!event.refresh) {
      emit(state.copyWith(
        notifications: state.notifications.copyWith(status: LoadStatus.loading),
      ));
    }

    try {
      final fetched = await repository.getNotifications(limit: event.limit, offset: event.offset);
      final combined = event.refresh ? fetched.notifications : [...current, ...fetched.notifications];
    ...
    try {
      ...
      emit(state.copyWith(
        ...
        lastFetchedAt: () => response.notifications.isNotEmpty
            ? response.notifications.first.createdAt.add(Duration(milliseconds: 1))
            : state.lastFetchedAt,
      ));
